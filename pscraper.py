...
def form_differences_list():
    ...
    form_diff_raw = list(map(lambda x: (x.find('span', attrs={'class': 'toctext'}).string), form_diffs))
    form_like_raw = list(map(lambda x: (x.find('span', attrs={'class': 'toctext'}).string), form_likes))
