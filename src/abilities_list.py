...
def scrape_abilities() -> List[Ability]:
    ...
    ability_html = [a.parent.parent for a in html.select('tr > td > a[class="ent-name"]')]
...
def add_to_database(client: MongoClient = None) -> None:
    ...
    client.get_database('test_database', codec_options=CODEC_OPTIONS).abilities.insert_many([i.asdict() for i in abilities])